...
class Dijkstra(dict):
    ...
    def __init__(
        self, node, neighbors, maxitems=10000, maxdist=None, is_target=None):
        ...
        Arguments:
            ...
            * is_target=None: optional function identifying the target node.
                The algorithm stops once a shortest path to the target has
                been found. (more below)
        ...
        for s in self._elements(heap, visited, maxitems, maxdist, is_target):
            if is_target(s.node):
                print('warning: target found in s.node case too')
            seen[s.node] = s
            visited.add(s.node)
    ...
    def _elements(self, heap, visited, maxitems, maxdist, is_target):
        ...
        if is_target is not None:
            seq = itt.takewhile((lambda x: not is_target(x.node)), seq)
        return seq
