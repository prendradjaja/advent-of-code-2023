...
def main():
    ...
    for i, card in enumerate(cards):
        print(f'Processing cards[{i}], which we have {card.count} of')
        if card.matches:
            print(f'- It has {card.matches} matches')
        next_idx = i + 1
        for j in range(next_idx, next_idx + card.matches):
            # Copy cards[j] once for every copy of cards[i]
            cards[j].count += card.count
            print(f'- Adding {card.count} copies of cards[{j}], for a total of {cards[j].count}')
        print()
